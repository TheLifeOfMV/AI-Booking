...
import { useAdminBookingsContext } from '@/platform/context/AdminBookingsProvider';
import { BookingStatus } from '@/domains/adminservice/types/admin';
    ...
    <div className="bg-white rounded-lg border border-light-grey shadow-sm p-4 mb-6">
      <div className="flex flex-col lg:flex-row lg:items-center gap-4">
        ...
        <div className="flex-grow">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Buscar reservas por ID, paciente o doctor..."
              value={searchTerm}
        ...
        <div className="hidden lg:flex space-x-3 items-center">
          <div className="flex items-center">
            <span className="text-sm text-dark-grey mr-2">Desde:</span>
            <input
              type="date"
          ...
          <div className="flex items-center">
            <span className="text-sm text-dark-grey mr-2">Hasta:</span>
            <input
              type="date"
              className="border border-light-grey rounded-md shadow-sm text-sm p-2 focus:outline-none focus:ring-primary focus:border-primary"
              value={endDate}
      ...
      {isExpanded && (
        <div className="mt-4 space-y-4 lg:hidden">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-dark-grey mb-1">Desde:</label>
              <input
                type="date"
            ...
            <div>
              <label className="block text-sm font-medium text-dark-grey mb-1">Hasta:</label>
              <input
                type="date"
                className="w-full border border-light-grey rounded-md shadow-sm text-sm p-2 focus:outline-none focus:ring-primary focus:border-primary"
                value={endDate}
          ...
          <div>
            <label className="block text-sm font-medium text-dark-grey mb-2">Estado:</label>
      ...
      {(selectedStatuses.length > 0 || (startDate && endDate) || searchTerm) && (
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <span className="text-xs text-medium-grey">Filtros activos:</span>
          ...
          {startDate && endDate && (
            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-light-grey text-dark-grey">
              ...
              >
                <span className="sr-only">Eliminar filtro de fecha</span>
                <svg className="h-2 w-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                  <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
                </svg>
              </button>
          ...
          {selectedStatuses.map((status) => (
            ...
            >
              Estado: {statusOptions.find(o => o.value === status)?.label || status}
              <button 
                type="button" 
                className="ml-1.5 inline-flex items-center justify-center h-4 w-4 rounded-full text-medium-grey hover:text-dark-grey focus:outline-none"
                onClick={() => toggleStatus(status)}
              >
                <span className="sr-only">Eliminar filtro {status}</span>
                <svg className="h-2 w-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                  <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
                </svg>
              </button>
