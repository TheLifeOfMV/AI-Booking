'use client';

import React from 'react';
import { useRouter } from 'next/navigation';
import Button from '@/components/Button';

export default function ChannelSelectionPage() {
  const router = useRouter();
  
  const handleChannelSelect = (channel: 'phone' | 'whatsapp' | 'app') => {
    if (channel === 'app') {
      router.push('/booking/insurance');
    } else {
      // For now, we just show an alert for phone/whatsapp options
      // This would be replaced with actual functionality in the future
      alert(`${channel} booking will be available in a future update.`);
    }
  };
  
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-white p-6 relative overflow-hidden">
      {/* Decorative wavy pattern for visual interest as shown in style guide */}
      <div className="absolute top-0 w-full h-12 opacity-20">
        <svg viewBox="0 0 800 80" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
          <path d="M0,40 Q80,80 160,40 T320,40 T480,40 T640,40 T800,40" stroke="#333333" fill="none" strokeWidth="2"/>
        </svg>
      </div>
      
      <div className="w-full max-w-md text-center mb-10">
        <h1 className="text-3xl font-bold text-dark-grey mb-3">
          Choose Your Booking Channel
        </h1>
        <p className="text-medium-grey text-lg">
          Select your preferred way to schedule your appointment
        </p>
      </div>
      
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-4xl">
        {/* Phone Call Card */}
        <div 
          onClick={() => handleChannelSelect('phone')}
          className="bg-white rounded-xl shadow-lg p-6 transition-all duration-300 hover:transform hover:scale-105 hover:shadow-xl cursor-pointer border border-light-grey"
        >
          <div className="w-16 h-16 bg-light-grey rounded-full flex items-center justify-center mx-auto mb-5">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
            </svg>
          </div>
          <h3 className="text-xl font-semibold text-dark-grey mb-3">Phone Call</h3>
          <p className="text-medium-grey mb-4">Speak with our virtual assistant to book your appointment</p>
          <Button type="text" className="mt-auto flex items-center justify-center mx-auto">
            Select <span className="ml-1">›</span>
          </Button>
        </div>
        
        {/* WhatsApp Card */}
        <div 
          onClick={() => handleChannelSelect('whatsapp')}
          className="bg-white rounded-xl shadow-lg p-6 transition-all duration-300 hover:transform hover:scale-105 hover:shadow-xl cursor-pointer border border-light-grey"
        >
          <div className="w-16 h-16 bg-light-grey rounded-full flex items-center justify-center mx-auto mb-5">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-primary" viewBox="0 0 24 24" fill="currentColor">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
            </svg>
          </div>
          <h3 className="text-xl font-semibold text-dark-grey mb-3">WhatsApp</h3>
          <p className="text-medium-grey mb-4">Chat with our AI assistant through WhatsApp</p>
          <Button type="text" className="mt-auto flex items-center justify-center mx-auto">
            Select <span className="ml-1">›</span>
          </Button>
        </div>
        
        {/* In-App Card */}
        <div 
          onClick={() => handleChannelSelect('app')}
          className="bg-white rounded-xl shadow-lg p-6 transition-all duration-300 hover:transform hover:scale-105 hover:shadow-xl cursor-pointer border border-light-grey relative overflow-hidden"
        >
          {/* Highlighted card with accent color indicator */}
          <div className="absolute top-0 right-0 w-0 h-0 border-t-[40px] border-r-[40px] border-t-primary border-r-transparent"></div>
          
          <div className="w-16 h-16 bg-light-grey rounded-full flex items-center justify-center mx-auto mb-5">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z" />
            </svg>
          </div>
          <h3 className="text-xl font-semibold text-dark-grey mb-3">In-App</h3>
          <p className="text-medium-grey mb-4">Book directly through our app interface</p>
          <Button type="primary" className="mt-auto flex items-center justify-center mx-auto">
            Book Now <span className="ml-1">›</span>
          </Button>
        </div>
      </div>
      
      {/* Bottom decorative wavy pattern */}
      <div className="absolute bottom-0 w-full h-12 opacity-20">
        <svg viewBox="0 0 800 80" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
          <path d="M0,40 Q80,0 160,40 T320,40 T480,40 T640,40 T800,40" stroke="#333333" fill="none" strokeWidth="2"/>
        </svg>
      </div>
    </div>
  );
} 